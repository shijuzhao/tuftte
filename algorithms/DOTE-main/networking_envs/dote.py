...
class DmDataset(Dataset):
    def __init__(self, props, env, is_test):
...
if props.so_mode == SOMode.TRAIN: #train
    ...
    #save the model
    torch.save(model, props.graph_base_path + '/' + props.ecmp_topo + '/' + 'model_dote.pkl')
    ...
    #load the model
    model = torch.load(props.graph_base_path + '/' + props.ecmp_topo + '/' + 'model_dote.pkl')
    model.eval()
    with torch.no_grad():
